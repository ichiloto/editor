 ...
 *
 * A cinematic or a summon is a pair: a data file and a script or timeline.
 * Installing one without the other is not a partial success, it is a broken
 * asset -- a definition with no script, or a script with no definition --
 * and the same is true of removing one without the other. This is the single
 * boundary through which a pair is written, first written, overwritten and
 * deleted, so every one of those paths ends with either the complete new
 ...
 * `PairedFileTransactionFailure::$wasRolledBack` is false and the files left
 ...
 */
final class PairedFileTransaction
{
    ...
    private bool $folderExisted = true;
    private bool $isStaged = false;
    private bool $isFinished = false;
    private int $temporaryCounter = 0;

    /**
     * @param string $folder The folder holding the pair.
     * @param PairedFileOperations $files The filesystem to act on.
     */
    public function __construct(
        private readonly string $folder,
        private readonly PairedFileOperations $files = new FilesystemPairedFileOperations(),
    ) {
    }
     ...
     * @throws PairedFileTransactionFailure When the proposal cannot be staged. Nothing is changed.
     */
    public function stage(): array
    {
        ...
        $this->folderExisted = $this->files->isDirectory($this->folder);
        ...
        foreach ($this->targets as $target) {
            ...
            if ($this->files->isFile($path)) {
                ...
                if ($contents === null) {
                    ...
                    throw new PairedFileTransactionFailure(sprintf('%s could not be read, so it could not be put back if the write failed', basename($path)));
                }
        ...
        if (! $this->folderExisted && ! $this->files->makeDirectory($this->folder)) {
            throw new PairedFileTransactionFailure(sprintf('Unable to create %s', $this->folder));
        }
        ...
        try {
            foreach ($this->targets as $target) {
                ...
                if (! $this->files->write($temporary, $contents)) {
                    throw new PairedFileTransactionFailure(sprintf('Unable to write a temporary file beside %s', basename($target['path'])));
                }

                $this->staged[$target['path']] = $temporary;

                if ($this->files->read($temporary) !== $contents) {
                    throw new PairedFileTransactionFailure(sprintf('The staged copy of %s did not read back as written', basename($target['path'])));
                }
            }
     ...
     * @throws PairedFileTransactionFailure When any target cannot be installed.
     */
     ...
     * @throws PairedFileTransactionFailure Always.
     */
    private function undo(array $installed, string $reason): never
    {
        ...
        throw new PairedFileTransactionFailure($reason, $unrestored === [], $unrestored);
    }
    ...
    {
        if ($this->folderExisted) {
            return;
        }

        $this->removeFolderIfEmpty();
    }
    ...
    private function removeFolderIfEmptied(): void
    {
        ...
        // Only the pair was this asset's; anything else in the folder is its
        // author's, and the folder goes only when nothing is left.
        $this->removeFolderIfEmpty();
    }

    private function removeFolderIfEmpty(): void
    {
