 ...
 * got.
 *
 * `$wasRolledBack` is true only when every path the transaction had already
 * touched was put back exactly as it was found. When a restoration itself
 * fails the failure says so and names the paths whose state on disk is not
...
final class FileSetTransactionFailure extends RuntimeException
{
     ...
     * @param bool $wasRolledBack Whether every touched path was restored.
     * @param string[] $unrestoredPaths The paths left in an unknown state.
     */
    ...
    private static function describe(string $reason, bool $wasRolledBack, array $unrestoredPaths): string
    {
        ...
        return sprintf(
            '%s, and the paths could not be put back: %s. Restore them from the backup before editing further.',
            $reason,
